...
场景库 Release `20260827-150658` 已上线，直接回滚版本为 `20260827-142935`。本版新增 99 个页面—模型绑定，连同既有 4 个样例共有 103 个模型展示页；菜单共有 20 个根节点、1,050 个总节点和 1,050 个唯一 ID。最终界面修订已移除模型页的“模型展示 / 原业务页”切换栏，绑定入口只呈现新开发的三维模型页面。

```text
current  → /nlplabProj/scene-library/releases/20260827-150658
previous → /nlplabProj/scene-library/releases/20260827-142935
health.version = 20260827-150658
```

`eq-18` 原页面及位置未改，新增的 `eq-unit1-model` 承接模型 `6691`；仿真分析前四页保持原位。场景库 HTTPS/HTTP、standalone、embedded、microapp、线上 99 绑定 API 验证均通过。主项目服务器没有在本轮修改；主项目团队仍需导入正式菜单并更新宿主模型详情白名单。完整命令证据、哈希和回滚信息见 `云端更新执行记录-20260827-模型页单页面修订.md`。后续接手必须以该记录和现场只读检查为准，不再以 2026-08-21 Release 作为当前生产基线。
