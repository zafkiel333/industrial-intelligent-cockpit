...
2026-08-20 最新状态：场景库 Release `20260820-201251` 已上线，回滚版本为 `20260820-195135`；HTTPS 独立入口、双构建资源、公开健康接口以及前四个外部模型页面的 `bootstrap` 均已通过外网 200 验证，HTTP 8081 兼容入口仍保留。本版增强“查看资源详情”超时诊断，但功能最终恢复仍需主平台修正消息 Origin：当前主平台同源 iframe `/cockpit/` 的真实来源是 `https://8.146.211.204:3100`，其运行代码仍硬编码旧值 `http://47.122.104.52:8081`。主平台还需按 `menuVersion=20260820-173922` 的 1049 节点清单给 20 个分类增加“首页”子菜单。联合验收完成前，不得宣布双平台导航拆分整体完成。精确热修见 `主平台统一导航拆分方案-20260820/09-模型详情跳转消息Origin热修说明.md`。
...
2026-08-20 Release `20260820-201251` 成功切换并完成技术验收后：

```text
current  → /nlplabProj/scene-library/releases/20260820-201251
previous → /nlplabProj/scene-library/releases/20260820-195135
```

健康检查返回：

```json
{"status":"ok","version":"20260820-201251"}
```
