...
2026-08-20 最新状态：场景库 Release `20260820-195135` 已上线；HTTPS 独立入口、双构建资源、公开健康接口以及前四个外部模型页面的 `bootstrap` 均已通过外网 200 验证，HTTP 8081 兼容入口仍保留。场景库 HTTPS 入口为 `https://47.122.104.52`，主平台不得继续嵌入旧 HTTP 地址。主平台合并页仍需完成两项自身整改：按 `menuVersion=20260820-173922` 的 1049 节点清单给 20 个分类增加“首页”子菜单；把 iframe/微应用入口、消息 origin 白名单和 CSP 切换为 HTTPS。联合验收完成前，不得宣布双平台导航拆分整体完成。
