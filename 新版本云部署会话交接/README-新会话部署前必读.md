...
2026-08-21 最新状态：场景库 Release `20260821-085852` 已上线，回滚版本为 `20260821-084944`。本版是“1号机组异常预测分析”3D 浮层二次修复：从 JSX 和 CSS 中彻底删除旧 `unit1-predictive-hud` 及深色背景类，两个面板只保留单层不透明浅色底 `#f1f7fa`；面板及所有后代文字先统一为深色，再为标签、时间、说明、计数、历史起点、当前计算、预测终点分别设置专用深色样式。双构建、3103 预检、现网 HTTPS/嵌入入口、公开 CSS/JS 正反向检查均通过。此前模型详情消息 Origin 的主平台热修及分类“首页”子菜单整改仍待主平台团队完成。
...
2026-08-21 Release `20260821-085852` 成功切换并完成技术验收后：

```text
current  → /nlplabProj/scene-library/releases/20260821-085852
previous → /nlplabProj/scene-library/releases/20260821-084944
```

健康检查返回：

```json
{"status":"ok","version":"20260821-085852"}
```
