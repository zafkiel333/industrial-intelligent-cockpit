...
import { ThreeScene } from '../../../components/Maintenance-plan-management/ElectricShovelMaintenance/ThreeScene';
// 2026-07-09 新增：模型库跳转链接（场景库测试方案 8.4）
import { ModelLibraryLink } from '../../../src/scenarioLib/ModelLibraryLink';
// MODEL_LIB_LINK[mpm-27]: 2026-07-09 新增，占位模型库地址；
// 模型库正式上线后，只需把下面这一行的 url 改成真实地址即可，其余逻辑不用动。
const MODEL_LIB_URL = 'https://industrial-intelligent-cockpit.example.com/model-lib/models/mpm-27';
...
export const ElectricShovelMaintenanceView: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-950 p-6">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-white tracking-wider">矿用电铲挖掘机定期维保</h1>
        <p className="text-slate-400 mt-2">智能维保管理系统 / 实时监控与调度</p>
      </div>
      
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <SciFiCard title="矿用电铲挖掘机定期维保 - 沉浸视图" className="h-[500px]">
        ...
        <div className="flex flex-col gap-6">
          <SciFiCard title="综合状态" className="h-[240px] overflow-y-auto">
            <ParameterWidget params={mockParams} />
          </SciFiCard>
          <SciFiCard title="资源与人员" className="h-[235px] overflow-y-auto">
            <ResourceWidget {...mockResources} />
          </SciFiCard>
        </div>
        <SciFiCard title="历史数据对比" className="h-[300px]">
          <ChartWidget data={mockChartData} type="bar" dataKey="value" color="#10b981" />
        </SciFiCard>
        <div className="flex gap-6">
          <SciFiCard title="关键节点" className="h-[300px] flex-1 overflow-y-auto">
            <TimelineWidget steps={mockTimeline} />
          </SciFiCard>
          <SciFiCard title="参考资料" className="h-[300px] flex-1 overflow-y-auto">
            <DocumentWidget docs={mockDocs} />
          </SciFiCard>
        </div>
      </div>
