...
import { ThreeScene } from '../../../components/computer-visual-inspection/HydraulicHose/ThreeScene';
// 2026-07-09 新增：模型库跳转链接（场景库测试方案 8.4）
import { ModelLibraryLink } from '../../../src/scenarioLib/ModelLibraryLink';
// MODEL_LIB_LINK[cv-hydraulic-hose]: 2026-07-09 新增，占位模型库地址；
// 模型库正式上线后，只需把下面这一行的 url 改成真实地址即可，其余逻辑不用动。
const MODEL_LIB_URL = 'https://industrial-intelligent-cockpit.example.com/model-lib/models/cv-hydraulic-hose';
import { HoseStatus } from '../../../components/computer-visual-inspection/HydraulicHose/three-types';
...
const mockHistoryData = Array.from({ length: 20 }, (_, i) => ({
  time: `${i}:00`,
  aging: 0.1 + (i / 40) + Math.random() * 0.05,
  pressure: 25 + Math.random() * 5,
  temp: 55 + Math.random() * 8
}));

const HydraulicHose: React.FC = () => {
  const [status, setStatus] = useState<HoseStatus>({
    pressure: 28.5,
    agingLevel: 0.35,
    leakDetected: false,
    leakPosition: { x: 0, y: 0, z: 0 },
    flowRate: 12.4,
    temperature: 58.2
  });
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setStatus(prev => {
        const isLeaking = Math.random() > 0.98;
        return {
          ...prev,
          pressure: 25 + Math.random() * 10,
          agingLevel: Math.min(1, prev.agingLevel + 0.0001),
          leakDetected: isLeaking,
          leakPosition: isLeaking ? { x: (Math.random() - 0.5) * 4, y: (Math.random() - 0.5) * 2, z: (Math.random() - 0.5) * 2 } : prev.leakPosition,
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-cyan-500/60 text-sm font-mono uppercase tracking-widest">
              Hydraulic Hose Aging & Leakage Visual Monitoring System
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-6">
          <div className="flex flex-col items-end">
            ...
            <div className="flex items-center gap-2">
              <div className={`w-2 h-2 rounded-full animate-pulse ${status.leakDetected ? 'bg-red-500 shadow-[0_0_8px_#ef4444]' : 'bg-emerald-500 shadow-[0_0_8px_#10b981]'}`} />
              <span className={`font-bold ${status.leakDetected ? 'text-red-400' : 'text-emerald-400'}`}>
                {status.leakDetected ? '检测到渗漏' : '运行正常'}
              </span>
            </div>
          </div>
              ...
              <div className="bg-black/40 backdrop-blur-md border border-white/10 p-4 rounded-xl">
                ...
                <div className="space-y-3">
                  <div className="flex justify-between items-center gap-8">
                    <span className="text-sm text-slate-400">老化指数</span>
                    <span className="text-lg font-mono text-white">{(status.agingLevel * 100).toFixed(1)}%</span>
                  </div>
                  <div className="flex justify-between items-center gap-8">
                    <span className="text-sm text-slate-400">系统压力</span>
              ...
              <div className="flex gap-4">
                <div className="bg-black/60 backdrop-blur-xl border border-cyan-500/30 p-4 rounded-2xl flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <div className="text-xl font-bold text-white">4K <span className="text-xs font-normal text-slate-500">UHD</span></div>
                  </div>
                </div>
              ...
              <div className="h-48">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={mockHistoryData}>
                    ...
                    />
                    <Area type="monotone" dataKey="aging" stroke="#06b6d4" fillOpacity={1} fill="url(#colorAging)" strokeWidth={2} />
                  </AreaChart>
                </ResponsiveContainer>
              </div>
            </SciFiCard>
              ...
              <div className="h-48">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={mockHistoryData}>
                    ...
                    />
                    <Line type="monotone" dataKey="pressure" stroke="#60a5fa" strokeWidth={2} dot={false} />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            </SciFiCard>
        ...
        <div className="col-span-12 lg:col-span-4 space-y-6">
          ...
          <SciFiCard className="p-6">
            ...
            <div className="grid grid-cols-1 gap-4">
              {[
                { label: '系统压力', value: status.pressure.toFixed(2), unit: 'MPa', icon: Gauge, color: 'text-blue-400', bg: 'bg-blue-500/10' },
                { label: '老化指数', value: (status.agingLevel * 100).toFixed(1), unit: '%', icon: Activity, color: 'text-cyan-400', bg: 'bg-cyan-500/10' },
                { label: '运行温度', value: status.temperature.toFixed(1), unit: '°C', icon: Thermometer, color: 'text-orange-400', bg: 'bg-orange-500/10' },
                { label: '瞬时流量', value: status.flowRate.toFixed(2), unit: 'L/min', icon: Droplets, color: 'text-emerald-400', bg: 'bg-emerald-500/10' },
              ].map((item, idx) => (
            ...
            <div className="space-y-4">
              <div className="p-4 bg-emerald-500/5 border border-emerald-500/20 rounded-xl">
                ...
                <p className="text-xs text-slate-400 leading-relaxed">
                  当前软管表面出现轻微龟裂，老化程度处于可控范围。建议每 30 天进行一次视觉复检。
                </p>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-start gap-3 text-xs text-slate-300">
                  ...
                  <span>检查接头处是否存在微量渗油</span>
                </div>
                <div className="flex items-start gap-3 text-xs text-slate-300">
                  <div className="w-1.5 h-1.5 rounded-full bg-cyan-500 mt-1.5 shrink-0" />
                  <span>监控压力脉动对软管疲劳的影响</span>
                </div>
              </div>
            </div>
          </SciFiCard>
          ...
          <SciFiCard className="p-6">
            ...
            <div className="space-y-3">
              {[
                { time: '15:45:12', msg: '检测到软管中部微量渗漏', level: 'error' },
                { time: '12:30:05', msg: '系统压力瞬时超过安全阈值', level: 'warning' },
                { time: '昨日 09:15', msg: '软管表面老化程度增加 2%', level: 'info' },
              ].map((alert, idx) => (
