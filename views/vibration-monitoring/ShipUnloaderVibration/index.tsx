...
import { ThreeScene } from '@/components/vibration-monitoring/ShipUnloaderVibration/ThreeScene';
// 2026-07-13 新增：模型库跳转链接（场景库测试方案 8.4）
import { ModelLibraryLink } from '@/src/scenarioLib/ModelLibraryLink';
// MODEL_LIB_LINK[vibe-ShipUnloaderVibration]: 2026-07-13 新增，占位模型库地址；
// 模型库正式上线后，只需把下面这一行的 url 改成真实地址即可，其余逻辑不用动。
const MODEL_LIB_URL = 'https://industrial-intelligent-cockpit.example.com/model-lib/models/vibe-ShipUnloaderVibration';
import { LineChart, Line, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, AreaChart, Area, BarChart, Bar } from 'recharts';
import { Activity, Zap, ShieldCheck, AlertCircle, Gauge, Truck, RotateCw, ArrowDownCircle, Layers, Wind } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import { UnloaderState } from '@/components/vibration-monitoring/ShipUnloaderVibration/three-types';

const mockData = Array.from({ length: 30 }, (_, i) => ({
  time: i,
  vibration: 2.5 + Math.sin(i * 0.5) * 1.2 + Math.random() * 0.6,
  hoistLoad: 60 + Math.sin(i * 0.3) * 15 + Math.random() * 5,
  temp: 45 + Math.random() * 10,
}));

const ShipUnloaderVibrationView: React.FC = () => {
  const [unloaderState, setUnloaderState] = useState<UnloaderState>({
    hoistSpeed: 1.2,
    trolleyPosition: 0.45,
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setUnloaderState(prev => ({
        ...prev,
        vibrationIntensity: 0.2 + Math.random() * 0.4,
        hoistSpeed: 1.0 + Math.random() * 0.5,
        trolleyPosition: (prev.trolleyPosition + 0.01) % 1.0,
        grabLoad: 25 + Math.random() * 10,
    ...
    <div className="flex flex-col h-full bg-[#020617] text-slate-200 font-[Rajdhani] overflow-hidden">
      ...
      <div className="flex items-center justify-between px-8 py-5 border-b border-cyan-500/20 bg-slate-900/40 backdrop-blur-xl relative overflow-hidden">
        ...
        <div className="flex items-center gap-6">
          ...
          <div>
            <h1 className="text-2xl font-black tracking-tighter text-white flex items-center gap-3">
              船舶卸船机智能振动监测
              <span className="text-[10px] bg-cyan-500/20 text-cyan-400 px-2 py-1 rounded border border-cyan-500/30 uppercase tracking-[0.2em] font-bold">Ship Unloader v4.0</span>
            </h1>
            <div className="text-xs text-slate-400 flex items-center gap-4 mt-1">
              <span className="flex items-center gap-1.5"><Activity size={14} className="text-cyan-500" /> 设备编号: SU-2026-P1</span>
              <span className="flex items-center gap-1.5"><ShieldCheck size={14} className="text-emerald-500" /> 运行状态: 正常卸载中</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-8">
          {[
            { label: '提升速度', val: unloaderState.hoistSpeed.toFixed(2), unit: 'm/s', color: 'text-cyan-400' },
            { label: '抓斗载荷', val: unloaderState.grabLoad.toFixed(1), unit: 't', color: 'text-yellow-400' },
            { label: '环境风速', val: unloaderState.windSpeed.toFixed(1), unit: 'm/s', color: 'text-emerald-400' },
          ].map((item, i) => (
      ...
      <div className="flex-1 grid grid-cols-12 gap-4 p-4 overflow-hidden">
        ...
        <div className="col-span-12 lg:col-span-8 flex flex-col gap-4 overflow-hidden">
          <SciFiCard 
            title="卸船机数字孪生可视化" 
            subtitle="UNLOADER DIGITAL TWIN" 
            className="flex-1 relative group"
            highlight
          >
            <div className="absolute inset-0 z-0">
              <ThreeScene state={unloaderState} />
            ...
            <div className="absolute inset-0 pointer-events-none p-6 flex flex-col justify-between z-10">
              <div className="flex justify-between items-start">
                ...
                >
                  <div className="text-[10px] text-slate-500 uppercase tracking-widest font-bold mb-1">主梁振动强度</div>
                  <div className="text-3xl font-mono font-black text-cyan-400">{(unloaderState.vibrationIntensity * 5.2).toFixed(2)} <span className="text-sm font-normal">mm/s</span></div>
                  <div className="w-full h-1 bg-slate-800 mt-2 rounded-full overflow-hidden">
                    <motion.div 
                      className="h-full bg-cyan-500"
                      animate={{ width: `${unloaderState.vibrationIntensity * 100}%` }}
                    />
                  </div>
                </motion.div>
                ...
                <div className="flex flex-col gap-2">
                  <div className="bg-slate-900/80 border border-slate-700/50 p-3 backdrop-blur-md rounded-xl flex items-center gap-3">
                    ...
                    <div>
                      <div className="text-[10px] text-slate-500 uppercase font-bold">电机负载</div>
              ...
              <div className="flex justify-center gap-4">
                <div className="bg-slate-900/80 border border-cyan-500/30 px-6 py-3 backdrop-blur-md rounded-full flex items-center gap-8">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-xs font-bold text-slate-300 uppercase">自动卸载模式</span>
                  </div>
                  <div className="w-px h-4 bg-slate-700" />
                  <div className="flex items-center gap-3">
                    <Wind className="text-emerald-400" size={18} />
                    <span className="text-xs font-bold text-slate-300 uppercase">风振补偿: 开启</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="grid grid-cols-4 gap-4 h-28">
            {[
              { label: '小车位置', val: (unloaderState.trolleyPosition * 100).toFixed(0), unit: '%', icon: Activity, color: 'text-cyan-400' },
              { label: '提升机压力', val: '18.5', unit: 'MPa', icon: Gauge, color: 'text-yellow-400' },
              { label: '驱动电机温', val: '52', unit: '°C', icon: Zap, color: 'text-orange-400' },
              { label: '钢丝绳张力', val: '95', unit: 'kN', icon: ShieldCheck, color: 'text-emerald-400' },
            ].map((stat, i) => (
              <motion.div 
                key={i}
        ...
        <div className="col-span-12 lg:col-span-4 flex flex-col gap-4 overflow-y-auto pr-1 custom-scrollbar">
          ...
          <AnimatePresence mode="wait">
            {activeTab === 'realtime' && (
              ...
              >
                <SciFiCard title="主梁振动趋势" subtitle="GANTRY VIBRATION TREND">
                ...
                <SciFiCard title="提升载荷分布" subtitle="HOIST LOAD DISTRIBUTION">
                  <div className="h-40 w-full">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={mockData.slice(-10)}>
                        ...
                        <XAxis dataKey="time" hide />
                        <YAxis hide />
                        <Bar dataKey="hoistLoad" fill="#eab308" radius={[4, 4, 0, 0]} />
                      </BarChart>
                    </ResponsiveContainer>
                  </div>
                </SciFiCard>
                  ...
                  <div className="space-y-4">
                    <div className="p-4 bg-cyan-950/20 border border-cyan-500/20 rounded-2xl">
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        <span className="text-xs font-bold text-cyan-400 uppercase">结构疲劳优化建议</span>
                      </div>
                      <p className="text-[11px] text-slate-400 leading-relaxed">
                        当前主梁在满载工况下存在 12Hz 的低频共振。建议优化小车加速曲线，将加速度降低 5%，以减少结构冲击响应，预计可延长主梁疲劳寿命 15%。
                      </p>
                    </div>

                    <div className="space-y-3">
                      {[
                        { label: '钢丝绳健康度', val: 85, color: 'bg-emerald-500' },
                        { label: '减速机齿轮状态', val: 92, color: 'bg-cyan-500' },
                        { label: '制动器磨损度', val: 18, color: 'bg-yellow-500' },
                      ].map((item, i) => (
                        <div key={i}>
...
export default ShipUnloaderVibrationView;
