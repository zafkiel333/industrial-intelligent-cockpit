...
const PortWinchView: React.FC = () => {
  return (
    <div className="p-6 space-y-6 overflow-y-auto h-full tech-grid-bg">
      <div className="flex justify-between items-center">
        <h1 className="text-2xl font-bold text-cyan-400 tracking-wider">港口系泊绞车排绳机构震动监测</h1>
        <div className="px-4 py-1 bg-cyan-900/30 border border-cyan-500/50 rounded text-xs text-cyan-300">震动监测核心场景</div>
      </div>
      <div className="grid grid-cols-12 gap-6">
        <div className="col-span-8 h-[600px]"><SciFiCard title="实时震动分析与3D数字孪生" className="h-full"><ThreeScene state={state} /></SciFiCard></div>
        <div className="col-span-4 space-y-6">
          ...
          <SciFiCard title="频谱分析摘要"><div className="text-sm text-slate-400 leading-relaxed">系统实时监测港口系泊绞车排绳机构震动监测的震动频率分布。当前主频稳定，未发现异常。</div></SciFiCard>
        </div>
      </div>
    </div>
  );
};
export default PortWinchView;
