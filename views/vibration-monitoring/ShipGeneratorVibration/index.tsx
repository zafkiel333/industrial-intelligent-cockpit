...
import { ThreeScene } from '@/components/vibration-monitoring/ShipGeneratorVibration/ThreeScene';
// 2026-07-13 新增：模型库跳转链接（场景库测试方案 8.4）
import { ModelLibraryLink } from '@/src/scenarioLib/ModelLibraryLink';
// MODEL_LIB_LINK[vibe-ShipGeneratorVibration]: 2026-07-13 新增，占位模型库地址；
// 模型库正式上线后，只需把下面这一行的 url 改成真实地址即可，其余逻辑不用动。
const MODEL_LIB_URL = 'https://industrial-intelligent-cockpit.example.com/model-lib/models/vibe-ShipGeneratorVibration';
...
import { Activity, Zap, ShieldCheck, AlertCircle, Gauge, Thermometer, Droplets, Settings, Power, Clock } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import { GeneratorState } from '@/components/vibration-monitoring/ShipGeneratorVibration/three-types';

const mockData = Array.from({ length: 30 }, (_, i) => ({
  time: i,
  vibration: 0.15 + Math.sin(i * 0.5) * 0.04 + Math.random() * 0.02,
  power: 800 + Math.sin(i * 0.2) * 100 + Math.random() * 20,
  temp: 80 + Math.random() * 5,
}));

const radarData = [
  { subject: '振动', A: 85, fullMark: 100 },
  { subject: '温度', A: 70, fullMark: 100 },
  { subject: '油压', A: 90, fullMark: 100 },
  { subject: '功率', A: 75, fullMark: 100 },
  { subject: '转速', A: 80, fullMark: 100 },
];

const ShipGeneratorVibrationView: React.FC = () => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setGenState(prev => ({
        ...prev,
        engineSpeed: 1490 + Math.random() * 20,
        vibrationIntensity: 0.15 + Math.random() * 0.1,
        outputPower: 800 + Math.random() * 100,
        coolantTemp: 78 + Math.random() * 8,
        oilPressure: 0.45 + Math.random() * 0.1,
      }));
    }, 2000);
    ...
    <div className="flex flex-col h-full bg-[#020617] text-slate-200 font-[Rajdhani] overflow-hidden">
      ...
      <div className="flex items-center justify-between px-8 py-5 border-b border-cyan-500/20 bg-slate-900/40 backdrop-blur-xl relative overflow-hidden">
        ...
        <div className="flex items-center gap-6">
          ...
          <div>
            <h1 className="text-2xl font-black tracking-tighter text-white flex items-center gap-3">
              船舶辅机发电机组振动监测
              <span className="text-[10px] bg-cyan-500/20 text-cyan-400 px-2 py-1 rounded border border-cyan-500/30 uppercase tracking-[0.2em] font-bold">Aux Generator v2.5</span>
            </h1>
            <div className="text-xs text-slate-400 flex items-center gap-4 mt-1">
              <span className="flex items-center gap-1.5"><Activity size={14} className="text-cyan-500" /> 机组编号: GEN-SET-03</span>
              <span className="flex items-center gap-1.5"><Clock size={14} className="text-emerald-500" /> 累计运行: 4250h</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-8">
          {[
            { label: '输出功率', val: genState.outputPower.toFixed(0), unit: 'kW', color: 'text-cyan-400' },
            { label: '机油压力', val: genState.oilPressure.toFixed(2), unit: 'MPa', color: 'text-yellow-400' },
            { label: '冷却水温', val: genState.coolantTemp.toFixed(1), unit: '°C', color: 'text-emerald-400' },
          ].map((item, i) => (
      ...
      <div className="flex-1 grid grid-cols-12 gap-4 p-4 overflow-hidden">
        ...
        <div className="col-span-12 lg:col-span-8 flex flex-col gap-4 overflow-hidden">
          <SciFiCard 
            title="发电机组数字孪生可视化" 
            subtitle="GENERATOR DIGITAL TWIN" 
            className="flex-1 relative group"
            highlight
          >
            <div className="absolute inset-0 z-0">
              <ThreeScene state={genState} />
            ...
            <div className="absolute inset-0 pointer-events-none p-6 flex flex-col justify-between z-10">
              <div className="flex justify-between items-start">
                ...
                >
                  ...
                  <div className="text-3xl font-mono font-black text-cyan-400">{(genState.vibrationIntensity * 10).toFixed(2)} <span className="text-sm font-normal">mm/s</span></div>
                  <div className="w-full h-1 bg-slate-800 mt-2 rounded-full overflow-hidden">
                    <motion.div 
                      className="h-full bg-cyan-500"
                      animate={{ width: `${genState.vibrationIntensity * 100}%` }}
                    />
                  </div>
                </motion.div>
                ...
                <div className="flex flex-col gap-2">
                  <div className="bg-slate-900/80 border border-slate-700/50 p-3 backdrop-blur-md rounded-xl flex items-center gap-3">
                    ...
                    <div>
                      <div className="text-[10px] text-slate-500 uppercase font-bold">引擎转速</div>
                      <div className="text-lg font-mono font-bold text-white">{genState.engineSpeed.toFixed(0)} <span className="text-xs opacity-40">RPM</span></div>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="flex justify-center gap-4">
                <div className="bg-slate-900/80 border border-cyan-500/30 px-6 py-3 backdrop-blur-md rounded-full flex items-center gap-8">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-xs font-bold text-slate-300 uppercase">电能质量: 优质</span>
                  </div>
                  <div className="w-px h-4 bg-slate-700" />
                  <div className="flex items-center gap-3">
                    <Activity className="text-emerald-400" size={18} />
                    <span className="text-xs font-bold text-slate-300 uppercase">负载率: 72%</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="grid grid-cols-4 gap-4 h-28">
            {[
              { label: '燃油消耗率', val: '215', unit: 'g/kWh', icon: Droplets, color: 'text-cyan-400' },
              { label: '排烟温度', val: '385', unit: '°C', icon: Thermometer, color: 'text-yellow-400' },
              { label: '励磁电流', val: '12.4', unit: 'A', icon: Zap, color: 'text-orange-400' },
              { label: '绝缘电阻', val: '150', unit: 'MΩ', icon: ShieldCheck, color: 'text-emerald-400' },
            ].map((stat, i) => (
              <motion.div 
                key={i}
        ...
        <div className="col-span-12 lg:col-span-4 flex flex-col gap-4 overflow-y-auto pr-1 custom-scrollbar">
          ...
          <AnimatePresence mode="wait">
            {activeTab === 'realtime' && (
              ...
              >
                <SciFiCard title="机组综合性能" subtitle="OVERALL PERFORMANCE">
                  <div className="h-48 w-full">
                    <ResponsiveContainer width="100%" height="100%">
                      <RadarChart cx="50%" cy="50%" outerRadius="80%" data={radarData}>
                        ...
                        <Radar name="机组" dataKey="A" stroke="#06b6d4" fill="#06b6d4" fillOpacity={0.5} />
                      </RadarChart>
                    </ResponsiveContainer>
                  </div>
                </SciFiCard>

                <SciFiCard title="功率输出趋势" subtitle="POWER OUTPUT TREND">
                  <div className="h-40 w-full">
                    <ResponsiveContainer width="100%" height="100%">
                      <AreaChart data={mockData}>
                        ...
                        />
                        <Area type="monotone" dataKey="power" stroke="#eab308" strokeWidth={2} fill="url(#powerGrad)" />
                      </AreaChart>
                    </ResponsiveContainer>
                  </div>
                </SciFiCard>
                  ...
                  <div className="space-y-4">
                    <div className="p-4 bg-emerald-950/20 border border-emerald-500/20 rounded-2xl">
                      ...
                      <p className="text-[11px] text-slate-400 leading-relaxed">
                        发电机组各项指标处于正常范围内。振动频谱分析未发现明显的轴承损坏或不平衡特征。建议维持当前负荷运行，并按计划在 250 小时后进行常规机油更换。
                      </p>
                    </div>

                    <div className="space-y-3">
                      {[
                        { label: '缸套磨损预测', val: 12, color: 'bg-emerald-500' },
                        { label: '轴承疲劳程度', val: 25, color: 'bg-cyan-500' },
                        { label: '励磁系统稳定性', val: 98, color: 'bg-yellow-500' },
                      ].map((item, i) => (
                        <div key={i}>
...
export default ShipGeneratorVibrationView;
