...
import { ThreeScene } from '@/components/vibration-monitoring/ShipFan/ThreeScene';
// 2026-07-13 新增：模型库跳转链接（场景库测试方案 8.4）
import { ModelLibraryLink } from '@/src/scenarioLib/ModelLibraryLink';
// MODEL_LIB_LINK[vibe-ShipFan]: 2026-07-13 新增，占位模型库地址；
// 模型库正式上线后，只需把下面这一行的 url 改成真实地址即可，其余逻辑不用动。
const MODEL_LIB_URL = 'https://industrial-intelligent-cockpit.example.com/model-lib/models/vibe-ShipFan';
import { LineChart, Line, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, AreaChart, Area } from 'recharts';
import { Activity, Zap, ShieldCheck, Thermometer, Layers, RotateCw, Settings, Wind, Fan } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import { ShipFanState } from '@/components/vibration-monitoring/ShipFan/three-types';

const mockData = Array.from({ length: 30 }, (_, i) => ({
  time: i,
  vibration: 0.12 + Math.sin(i * 0.5) * 0.03 + Math.random() * 0.02,
  flow: 8500 + Math.random() * 500,
  pressure: 1200 + Math.random() * 100,
}));

const ShipFanView: React.FC = () => {
  const [state, setState] = useState<ShipFanState>({
    fanSpeed: 1250,
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setState(prev => ({
        ...prev,
        fanSpeed: 1000 + Math.random() * 500,
        vibrationIntensity: 0.1 + Math.random() * 0.15,
        motorTemp: 40 + Math.random() * 15,
        airFlow: 8000 + Math.random() * 1000,
        pressure: 1100 + Math.random() * 300,
      }));
    }, 2000);
    ...
    <div className="flex flex-col h-full bg-[#020617] text-slate-200 font-[Rajdhani] overflow-hidden">
      <div className="flex items-center justify-between px-8 py-5 border-b border-cyan-500/20 bg-slate-900/40 backdrop-blur-xl relative overflow-hidden">
        ...
        <div className="flex items-center gap-6">
          <motion.div animate={{ rotate: [0, 360] }} transition={{ duration: 5, repeat: Infinity, ease: "linear" }} className="w-14 h-14 rounded-2xl bg-cyan-500/10 flex items-center justify-center border border-cyan-500/30 shadow-[0_0_20px_rgba(6,182,212,0.2)]">
            <Fan className="text-cyan-400" size={28} />
          </motion.div>
          <div>
            <h1 className="text-2xl font-black tracking-tighter text-white flex items-center gap-3 uppercase">船舶通风机震动监测<span className="text-[10px] bg-cyan-500/20 text-cyan-400 px-2 py-1 rounded border border-cyan-500/30 uppercase tracking-[0.2em] font-bold">Ship Ventilator v1.8</span></h1>
            <div className="text-xs text-slate-400 flex items-center gap-4 mt-1">
              <span className="flex items-center gap-1.5"><Activity size={14} className="text-cyan-500" /> 设备编号: FAN-SHIP-04</span>
              <span className="flex items-center gap-1.5"><Layers size={14} className="text-emerald-500" /> 运行状态: 运行中</span>
            </div>
          </div>
        </div>
        <div className="flex items-center gap-8">
          {[
            { label: '风机转速', val: state.fanSpeed.toFixed(0), unit: 'RPM', color: 'text-cyan-400' },
            { label: '风量', val: state.airFlow.toFixed(0), unit: 'm³/h', color: 'text-yellow-400' },
            { label: '静压', val: state.pressure.toFixed(0), unit: 'Pa', color: 'text-emerald-400' },
          ].map((item, i) => (
      ...
      <div className="flex-1 grid grid-cols-12 gap-4 p-4 overflow-hidden">
        <div className="col-span-12 lg:col-span-8 flex flex-col gap-4 overflow-hidden">
          <SciFiCard title="通风机数字孪生可视化" subtitle="SHIP VENTILATOR DIGITAL TWIN" className="flex-1 relative group" highlight>
            ...
            <div className="absolute inset-0 pointer-events-none p-6 flex flex-col justify-between z-10">
              <div className="flex justify-between items-start">
                ...
                <div className="bg-slate-900/80 border border-slate-700/50 p-3 backdrop-blur-md rounded-xl flex items-center gap-3">
                  ...
                  <div>
                    <div className="text-[10px] text-slate-500 uppercase font-bold">电机温度</div>
                    <div className="text-lg font-mono font-bold text-white">{state.motorTemp.toFixed(1)} <span className="text-xs opacity-40">°C</span></div>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="grid grid-cols-4 gap-4 h-28">
            {[
              { label: '风机效率', val: '82', unit: '%', icon: Activity, color: 'text-cyan-400' },
              { label: '电机电流', val: '45', unit: 'A', icon: Zap, color: 'text-yellow-400' },
              { label: '叶片平衡', val: '0.05', unit: 'mm', icon: Activity, color: 'text-orange-400' },
              { label: '系统健康度', val: '98', unit: '%', icon: ShieldCheck, color: 'text-emerald-400' },
            ].map((stat, i) => (
          ...
          <AnimatePresence mode="wait">
            {activeTab === 'realtime' && (
              <motion.div key="realtime" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="flex flex-col gap-4">
                <SciFiCard title="轴承振动波形" subtitle="BEARING VIBRATION">
                ...
                <SciFiCard title="风量趋势" subtitle="AIR FLOW TREND">
                  <div className="h-40 w-full">
                    <ResponsiveContainer width="100%" height="100%">
                      <LineChart data={mockData}>
                        ...
                        <Line type="monotone" dataKey="flow" stroke="#eab308" strokeWidth={2} dot={false} />
                      </LineChart>
                    </ResponsiveContainer>
                  </div>
                </SciFiCard>
              ...
              <motion.div key="health" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-4">
                <SciFiCard title="智能诊断报告" subtitle="AI DIAGNOSTICS">
                  <div className="p-4 bg-cyan-950/20 border border-cyan-500/20 rounded-2xl">
                    ...
                    <p className="text-[11px] text-slate-400 leading-relaxed">通风机运行稳定。振动频谱未发现明显的叶片失衡或轴承故障特征。风量与静压输出符合设计工况，建议维持当前运行状态。</p>
                  </div>
                </SciFiCard>
              </motion.div>
            )}
