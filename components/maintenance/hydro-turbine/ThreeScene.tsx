...
import { initHydroTurbineScene, animateHydroScene } from './HydroTurbineBuilder';
...
export const ThreeScene: React.FC<ThreeSceneProps> = ({ step }) => {
  ...
  useEffect(() => {
    ...
    const spotLight = new THREE.SpotLight(0x0ea5e9, 5); // Blue industrial accent
    ...
    // Initialize Builder
    const animatables: HydroAnimatables = {};
    ...
    const animate = () => {
      ...
      controls.update();
      animateHydroScene(animatables, step, time);
  ...
  }, [step]); // Re-run mainly if step logic needs profound reset, but usually handled by animate loop props
