...
const SMART_OPS_TYPES: SceneType[] = [
...
export const isSmartOpsScene = (type: SceneType): boolean => {
  return SMART_OPS_TYPES.includes(type);
};

export const setupSmartOpsCamera = (camera: THREE.PerspectiveCamera, type: SceneType) => {
  if (type === 'generator') {
...
export const initSmartOpsScene = (
  type: SceneType, 
  group: THREE.Group, 
  animatables: Animatables, 
  disposables: { dispose: () => void }[],
  materials: { wireframe: THREE.Material, solid: THREE.Material }
) => {
  const { wireframe: wireframeMaterial, solid: solidMaterial } = materials;
...
export const animateSmartOpsScene = (type: SceneType, animatables: Animatables, time: number) => {
  ...
  if (type === 'mine-hoist') {
      ...
      const yPos = Math.sin(progress * Math.PI * 2) * 2; 
