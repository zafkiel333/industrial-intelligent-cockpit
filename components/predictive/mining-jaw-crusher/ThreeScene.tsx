...
export const JawCrusherThreeScene: React.FC<JawCrusherSceneProps> = ({
  ...
  // 2026.03.04 修复模型闪烁问题 - 新增ref存储实时值，避免依赖项触发useEffect重渲染
  // Bug原因：原useEffect依赖state/isRunning/viewMode，这些变量频繁变化会导致useEffect反复执行，
  // 重新创建3D场景、渲染器等资源，引发模型闪烁
  const stateRef = useRef(state);
  const isRunningRef = useRef(isRunning);
  const viewModeRef = useRef(viewMode);

  // 监听变量变化，更新ref值（不会触发3D场景重建）
  useEffect(() => {
  ...
  // viewMode变化时更新材质（不重建场景）
  useEffect(() => {
    viewModeRef.current = viewMode;
    if (!sceneRef.current) return;

    // 更新颚板磨损材质
    const machineGroup = sceneRef.current.children.find(child => child.type === 'Group') as THREE.Group;
    if (!machineGroup) return;

    // 固定颚板
    const fixedPlate = machineGroup.children.find(
      child => child.position.x === 1.2 && child.position.y === 3
    ) as THREE.Mesh;
    // 活动颚板
    const movPlate = (movableJawRef.current?.children.find(
      child => child.position.x === 0.8 && child.position.y === -2.5
    )) as THREE.Mesh;
    // 框架材质
    const frameObjects = machineGroup.children.filter(
      child => ['Mesh', 'Group'].includes(child.type) && child.userData.id !== 'fixed-jaw'
    ) as THREE.Mesh[];

    if (fixedPlate && movPlate) {
      const jawPlateMat = new THREE.MeshStandardMaterial({
        color: 0x57534e, metalness: 0.3, roughness: 0.9
      });
      const wearMat = new THREE.MeshStandardMaterial({
        color: 0xff0000, metalness: 0.2, roughness: 0.5
      });
      fixedPlate.material = viewMode === 'wear' ? wearMat : jawPlateMat;
      movPlate.material = viewMode === 'wear' ? wearMat : jawPlateMat;
    }

    // 更新框架透明度（应力视图）
    frameObjects.forEach(obj => {
      if (obj.material instanceof THREE.MeshStandardMaterial) {
        obj.material.transparent = viewMode === 'stress';
        obj.material.opacity = viewMode === 'stress' ? 0.3 : 1.0;
        obj.material.needsUpdate = true;
      }
    });
  }, [viewMode]);

  // 2026.03.04 修复模型闪烁问题 - 剔除依赖项，仅初始化一次3D场景
  // Bug情况：原useEffect依赖项反复变化导致场景重复创建，引发模型闪烁
  // 修复方案：依赖项改为空数组，通过ref读取实时值，避免重复初始化
  useEffect(() => {
    ...
    const frameMat = new THREE.MeshStandardMaterial({ 
      ...
      transparent: viewModeRef.current === 'stress', opacity: viewModeRef.current === 'stress' ? 0.3 : 1.0
    });
    ...
    const fixedPlate = new THREE.Mesh(fixedPlateGeo, viewModeRef.current === 'wear' ? wearMat : jawPlateMat);
    ...
    const movPlate = new THREE.Mesh(movPlateGeo, viewModeRef.current === 'wear' ? wearMat : jawPlateMat);
    ...
    const animate = () => {
      ...
      // 读取实时isRunning状态（从ref获取，避免依赖项）
      if (isRunningRef.current) {
          // Eccentric Motion
          const ecc = 0.15;
          // 读取实时state.rpm值
          const speed = stateRef.current.rpm / 60 * 0.5;
  ...
  }, []); // 2026.03.04 修复模型闪烁问题 - 剔除所有依赖项，仅初始化一次
